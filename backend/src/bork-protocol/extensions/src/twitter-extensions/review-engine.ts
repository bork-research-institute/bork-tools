...
const TweetReviewSchema = z.object({
  ...
  reasoning: z.string().describe('Detailed explanation of the review decision'),
  suggestedEdits: z
    .string()
    .optional()
    .nullable()
...
export async function startTweetReviewEngine(runtime: AgentRuntime) {
  ...
  const reviewTweets = async () => {
    ...
    if (pendingTweets.length === 0) {
      elizaLogger.info('No pending tweets found. Waiting for new tweets...');
      return;
    }
    elizaLogger.info(
    ...
    for (const tweet of pendingTweets) {
      try {
        const review = await reviewTweet(tweet.text, runtime, recentTweetsText);
        ...
        elizaLogger.info(`Tweet ${tweet.id} reviewed: ${review.status}`);
      } catch (error) {
