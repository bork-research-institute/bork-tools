import { elizaLogger, generateMessageResponse } from '@elizaos/core';
...
export class TwitterSearchClient extends ClientBase {
  ...
  private async processAndStoreTweet(
    ...
    try {
      ...
      let analysis: { text: string } | undefined;
      let retryCount = 0;
      const maxRetries = 3;
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          elizaLogger.debug('[Twitter Search] Generated template:', {
          ...
          elizaLogger.debug(
            '[Twitter Search] Calling generateMessageResponse with:',
            {
              hasRuntime: Boolean(this.runtime),
              contextLength: template.context.length,
              modelClass: template.modelClass,
            },
          );
          ...
          elizaLogger.debug('[Twitter Search] Raw analysis response:', {
            analysis,
          ...
          if (!analysis || typeof analysis !== 'object' || !analysis.text) {
            throw new Error('Invalid analysis response structure');
          }

          elizaLogger.debug('[Twitter Search] Received analysis response:', {
            rawResponse: analysis.text,
            responseLength: analysis.text.length,
          });

          // Clean and validate the response
          const cleanedText = this.extractJsonFromText(analysis.text);
          ...
          elizaLogger.error('[Twitter Search] Analysis error details:', {
            error: error.message,
            stack: error.stack,
            rawResponse: analysis?.text || 'No response received',
            attempt: retryCount,
            maxRetries,
          });
      ...
      const tweetAnalysis = JSON.parse(cleanedText) as TweetAnalysis;
      await tweetQueries.insertTweetAnalysis(
        tweet.id,
        tweetAnalysis.spamAnalysis.isSpam
          ? 'engagement'
          : tweetAnalysis.contentAnalysis.type,
        tweetAnalysis.spamAnalysis.isSpam
          ? 'neutral'
          ...
          : tweetAnalysis.contentAnalysis.confidence,
        {
          ...
          spamScore: tweetAnalysis.spamAnalysis.spamScore,
          spamViolations: tweetAnalysis.spamAnalysis.reasons,
          ...(tweetAnalysis.spamAnalysis.isSpam
            ? {}
            : tweetAnalysis.contentAnalysis.metrics),
        },
        tweetAnalysis.spamAnalysis.isSpam
          ? []
          : tweetAnalysis.contentAnalysis.entities,
        tweetAnalysis.spamAnalysis.isSpam
          ? []
          : tweetAnalysis.contentAnalysis.topics,
        tweetAnalysis.spamAnalysis.isSpam
          ? 0.1
          : tweetAnalysis.contentAnalysis.impactScore,
        tweet.created_at,
        tweet.author_id,
        tweet.text,
      ...
      elizaLogger.info(
        ...
        {
          type: tweetAnalysis.spamAnalysis.isSpam ? 'spam' : 'content',
          author: tweet.author_id,
          analysisType: tweetAnalysis.spamAnalysis.isSpam
            ? 'engagement'
            : tweetAnalysis.contentAnalysis.type,
          sentiment: tweetAnalysis.spamAnalysis.isSpam
            ? 'neutral'
            : tweetAnalysis.contentAnalysis.sentiment,
          topicCount: tweetAnalysis.spamAnalysis.isSpam
            ? 0
            : tweetAnalysis.contentAnalysis.topics.length,
          entityCount: tweetAnalysis.spamAnalysis.isSpam
            ? 0
            : tweetAnalysis.contentAnalysis.entities.length,
        },
      );
    } catch (error) {
      elizaLogger.error('[Twitter Search] Error processing tweet:', error);
    }
  }
